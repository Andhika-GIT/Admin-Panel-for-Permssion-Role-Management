...
class RolesAndPermissionsSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $superAdmin->assignRole($superAdminRole)->syncPermissions(Permission::all());

        $admin->assignRole($adminRole)->syncPermissions(Permission::all());

        $moderator->assignRole($moderatorRole)->syncPermissions([
            $userPermission2,
            $rolePermission2,
            $permission2,
            $adminPermission1,
        ...
        $developer->assignRole($developerRole)->syncPermissions([
            $adminPermission1,
        ]);

        $user->assignRole($userRole)->syncPermissions($miscPermission);



        // $superAdmin->syncRoles($superAdminRole)->syncPermissions(Permission::all());

        // $admin->syncRoles($adminRole)->syncPermissions(Permission::all());

        // $moderator->syncRoles($moderatorRole)->syncPermissions($moderatorRole);

        // $developer->syncRoles($developerRole)->syncPermissions($developerRole);

        // $user->syncRoles($userRole)->syncPermissions($userRole);
    }
}
